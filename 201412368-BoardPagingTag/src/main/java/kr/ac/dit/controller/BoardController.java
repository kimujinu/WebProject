...
public class BoardController {
	...
	private BoardService boardService;
	 ...
	 public String listGET(@PathVariable int page, Model model, HttpSession httpSession) throws Exception {
	  ...
	  System.out.print("BoardController->page(현재 페이지 번호): " + page);
	  ...
	  int totalPage = 0;
	  if (totalCount>0) totalPage = (int)Math.ceil(totalCount/10.0); // 전체 페이지 개수 , math.ceil = 자리 올림
	  model.addAttribute("totalPageCount", totalPage); //list.jsp에 넘겨줌 
	  model.addAttribute("page", page); // 얘도
